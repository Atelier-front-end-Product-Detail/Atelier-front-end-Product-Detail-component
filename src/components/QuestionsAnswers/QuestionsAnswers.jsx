...
function QuestionsAnswers({ bridge, productId, productName }) {
  ...
  const handleSearch = (searchValue) => {
    ...
    if (searchValue.length >= 3) {
      const filtered = data.filter((entry) => entry.question_body.toString().toLowerCase().includes(searchValue.toLowerCase()));
  ...
  return !data ? (
    ...
    <div className="questions-answers-overview">
      ...
      <QuestionModal productId={productId} handlePostQuestion={handlePostQuestion} handleShowModal={handleShowModal} showQuestionModal={showQuestionModal} />
